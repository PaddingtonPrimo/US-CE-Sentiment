...
const StatCard = ({ value, label, delta, deltaLabel, color = "#e2e8f0" }) => (
  <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 12, padding: "20px 24px", flex: 1, minWidth: 180 }}>
    <div style={{ fontSize: 32, fontWeight: 800, color, fontFamily: "'Space Mono', monospace", letterSpacing: "-1px" }}>{value}</div>
    <div style={{ fontSize: 12, color: "#64748b", marginTop: 4, fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.5px" }}>{label}</div>
    {delta && (
      <div style={{ fontSize: 12, color: delta.startsWith("+") ? "#10b981" : "#ef4444", marginTop: 8, fontWeight: 600 }}>
...
const PhaseLabel = ({ phase, color }) => (
  <span style={{ display: "inline-block", padding: "3px 10px", borderRadius: 100, fontSize: 11, fontWeight: 600, background: color + "18", color: color, textTransform: "uppercase", letterSpacing: "0.5px" }}>{phase}</span>
);
    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: "#0a0a0f", color: "#e2e8f0", minHeight: "100vh", padding: "32px 24px", maxWidth: 960, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 600, color: "#ef4444", textTransform: "uppercase", letterSpacing: "1.5px" }}>Case Study</span>
        </div>
        ...
        <p style={{ color: "#64748b", fontSize: 14, margin: "8px 0 0", lineHeight: 1.5 }}>Jan 2025 - Mar 2026 - Tracking the tariff-driven confidence collapse and its impact on consumer electronics</p>
      </div>
      <div style={{ display: "flex", gap: 4, marginBottom: 28, background: "rgba(255,255,255,0.03)", borderRadius: 10, padding: 4, flexWrap: "wrap" }}>
        {tabs.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ padding: "8px 16px", borderRadius: 8, border: "none", cursor: "pointer", fontSize: 13, fontWeight: 600, fontFamily: "'DM Sans', sans-serif", transition: "all 0.2s", background: activeTab === tab.id ? "rgba(255,255,255,0.1)" : "transparent", color: activeTab === tab.id ? "#e2e8f0" : "#64748b" }}>{tab.label}</button>
        ))}
      </div>
      ...
      {activeTab === "overview" && (
        <div>
          ...
          <div style={{ background: "linear-gradient(135deg, rgba(239,68,68,0.06), rgba(245,158,11,0.04))", border: "1px solid rgba(239,68,68,0.12)", borderRadius: 14, padding: "24px 28px", marginBottom: 28 }}>
            <h3 style={{ fontSize: 15, fontWeight: 700, margin: "0 0 12px", color: "#f59e0b" }}>The Story in Five Phases</h3>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {[
                ...
                <div key={i} style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
                  <div style={{ minWidth: 120, paddingTop: 2 }}><PhaseLabel phase={p.phase} color={p.color} /></div>
                  <div>
          ...
          <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 14, padding: "20px 20px 8px" }}>
            <h3 style={{ fontSize: 13, fontWeight: 700, margin: "0 0 16px", color: "#94a3b8" }}>Michigan Consumer Sentiment Index - Jan 2025-Feb 2026</h3>
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={sentimentData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                <defs><linearGradient id="sentGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#f59e0b" stopOpacity={0.3} /><stop offset="100%" stopColor="#f59e0b" stopOpacity={0} /></linearGradient></defs>
      ...
      {activeTab === "sentiment" && (
        <div>
          <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 14, padding: "24px 24px 12px", marginBottom: 20 }}>
            ...
            <p style={{ fontSize: 12, color: "#64748b", margin: "0 0 20px" }}>University of Michigan vs Conference Board</p>
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={sentimentData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)" />
                <XAxis dataKey="month" tick={{ fill: "#475569", fontSize: 10 }} tickLine={false} axisLine={{ stroke: "rgba(255,255,255,0.06)" }} />
                <YAxis yAxisId="left" domain={[45, 80]} tick={{ fill: "#475569", fontSize: 10 }} tickLine={false} axisLine={false} />
                <YAxis yAxisId="right" orientation="right" domain={[70, 110]} tick={{ fill: "#475569", fontSize: 10 }} tickLine={false} axisLine={false} />
                <Tooltip content={<CustomTooltip />} />
          ...
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
            {[
              { title: "Peak-to-Trough Drop", value: "-28%", detail: "Michigan index fell from 73.2 to 50.8", color: "#ef4444" },
              { title: "Wealth Divide", value: "2x", detail: "Stockholders recovered; non-stockholders stagnated", color: "#f59e0b" },
              { title: "Inflation Expectations", value: "6.5%", detail: "One-year-ahead expected inflation highest since 1980s", color: "#ef4444" },
              { title: "Partisan Gap", value: "73pt", detail: "Republicans ~100; Democrats/Independents ~70", color: "#8b5cf6" },
            ].map((card, i) => (
      ...
      {activeTab === "spending" && (
        <div>
          <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 14, padding: "24px 24px 12px", marginBottom: 20 }}>
            ...
            <p style={{ fontSize: 12, color: "#64748b", margin: "0 0 20px" }}>Source: Circana POS data</p>
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={ceSpendingData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                ...
                <Bar dataKey="growth" name="YoY Growth %" radius={[6, 6, 0, 0]} barSize={56}>
                  {ceSpendingData.map((entry, i) => (<Cell key={i} fill={entry.color} fillOpacity={0.85} />))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
      ...
      {activeTab === "categories" && (
        <div>
          <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 14, padding: "24px 24px 12px", marginBottom: 20 }}>
            <h3 style={{ fontSize: 14, fontWeight: 700, margin: "0 0 4px" }}>Tariff-Driven Price Increases vs Purchase Decline</h3>
            <p style={{ fontSize: 12, color: "#64748b", margin: "0 0 20px" }}>By CE product category</p>
            <ResponsiveContainer width="100%" height={300}>
      ...
      {activeTab === "timeline" && (
        <div>
          ...
          <div style={{ marginTop: 20, padding: "16px 20px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 12 }}>
            ...
            <p style={{ fontSize: 11, color: "#64748b", margin: 0, lineHeight: 1.6 }}>University of Michigan Surveys of Consumers, The Conference Board, McKinsey ConsumerWise, Circana, CTA, Deloitte, Morning Consult, NielsenIQ</p>
          </div>
        </div>
      )}
